/****************************************************************************************************************************************
/*File Name : Inter-Process Communication using Shared Memory, Process 1
/*Author : Vatsal Sheth											
/*Dated 2/28/2019												
...
int main()
{
	...
	fprintf(fp, "\n[%ld : %ld : %ld : %ld] Process 1 Main Thread IPC: Shared Memory:\nProcess ID: %d\n\n",diff.tv_sec, (diff.tv_nsec / 1000000), (diff.tv_nsec / 1000), diff.tv_nsec, getpid());
	
	shm_fd = shm_open(mem_name, O_CREAT | O_RDWR, 0664);
	if(shm_fd == -1)
		handle_error("shm_open");
	
	sem1_fd = sem_open(sem1_name, O_CREAT, 0664, 0);
	if(sem1_fd == SEM_FAILED)
		handle_error("sem_open");
		
	sem2_fd = sem_open(sem2_name, O_CREAT, 0664, 0);
