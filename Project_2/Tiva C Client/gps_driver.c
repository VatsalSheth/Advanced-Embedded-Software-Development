 ...
 * Description: This file has API for GPS
 * Date: 4/29/2019
...
byte _parity;
boole _is_checksum_term;
char _term[15];
byte _sentence_type;
byte _term_number = 0;
byte _term_offset = 0;
boole _is_gps_data_good;
...
boole gpsisdigit(char c) { return c >= '0' && c <= '9'; }
 ...
 * @return
 */
boole gps_encode(char c)
{
    boole valid_sentence = false;
...
/**
 * @brief Processes a just-completed term
 * @return Returns true if new sentence has just passed checksum test and is validated
 */
...
/**
 * @brief lat/long in hundred thousandth of a degree and age of fix in milliseconds
 * @param latitude Pointer to write latitude value
 * @param longitude Pointer to write longitude value
 * @param fix_age
 */
...
/**
 * @brief Date as ddmmyy, time as hhmmsscc, and age in milliseconds
 * @param date Pointer to return date value
 * @param time Pointer to return time value
 * @param age Pointer to return age value
 */
